...
# [GCC 9.4.0]
# Embedded file name: libs/bl60x/img_create_do.py
...
def img_update_efuse_sp(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel, sec_eng_key):
...
def img_update_efuse_cpu0(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel, sec_eng_key):
...
def img_update_efuse_cpu1(cfg, sign, pk_hash, flash_encryp_type, flash_key, sec_eng_key_sel, sec_eng_key):
    ...
    if encrypt != 0:
        ...
        if encrypt != 0:
            ...
            if flash_img == 1:
                ...
                if encrypt != 0:
                    if encrypt == 1:
                        img_update_efuse_fun(cfg, sign, pk_hash, 1, encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
                    if encrypt == 2:
                        img_update_efuse_fun(cfg, sign, pk_hash, 3, encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
                    if encrypt == 3:
                        img_update_efuse_fun(cfg, sign, pk_hash, 2, encrypt_key + bytearray(32 - len(encrypt_key)), key_sel, None)
                else:
                ...
                fp.close()
                if encrypt != 0:
                    img_update_efuse_fun(cfg, sign, pk_hash, 1, None, key_sel, encrypt_key + bytearray(32 - len(encrypt_key)))
                else:
...
def img_create_do(args, img_dir_path=None, config_file=None):
    ...
    if ret0 == 'OK' and ret1 == 'OK':
        if signer == 'cpu0':
            ...
            if flash_img == 1:
                sign_process(cfg, data_tohash_cpu1, cfg.get('Img_CPU1_Cfg', 'bootinfo_file'), cfg.get('Img_CPU0_Cfg', 'privatekey_file_uecc'), cfg.get('Img_CPU0_Cfg', 'publickey_file'), 'cpu0')
            else:
                sign_process(cfg, data_tohash_cpu1, cfg.get('Img_CPU1_Cfg', 'whole_img_file'), cfg.get('Img_CPU0_Cfg', 'privatekey_file_uecc'), cfg.get('Img_CPU0_Cfg', 'publickey_file'), 'cpu0')
        else:
            if signer == 'cpu1':
                bflb_utils.printf('cpu1 sign cpu0 ')
                if flash_img == 1:
                    sign_process(cfg, data_tohash_cpu0, cfg.get('Img_CPU0_Cfg', 'bootinfo_file'), cfg.get('Img_CPU1_Cfg', 'privatekey_file_uecc'), cfg.get('Img_CPU1_Cfg', 'publickey_file'), 'cpu1')
                else:
                    sign_process(cfg, data_tohash_cpu0, cfg.get('Img_CPU0_Cfg', 'whole_img_file'), cfg.get('Img_CPU1_Cfg', 'privatekey_file_uecc'), cfg.get('Img_CPU1_Cfg', 'publickey_file'), 'cpu1')
    else:
